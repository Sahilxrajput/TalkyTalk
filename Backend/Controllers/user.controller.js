...
module.exports.signUpUser = async (req, res) => {
    try{
        ...
        const {username, firstName, confirmPassword, otp, lastName, email, age, bio, password, gender} = req.body;
        ...
        const newuser = new User({email, gender, age, bio, firstName, lastName, username});
        ...
        User.register(newuser, password, async(err, user)=>{
            ...
            req.login(user, (LoginErr)=>{
                if(LoginErr){
                    ...
                    return res.status(500).json({error: "Registration successful but automatic login failed"});
                }
...
module.exports.loginUser = async(req,res,next)=>{
    req.logout((err)=>{
        if(err){
            return res.status(400).json({ error: "current login user is not logout"})
        }
    })
...
module.exports.updateUserInfo = async (req, res) => {
    try {
        ...
        const updatedUser = await User.findByIdAndUpdate(id, { username, bio }, { new: true });  // `new: true` returns the updated document
